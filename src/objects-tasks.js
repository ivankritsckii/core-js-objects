...
function mergeObjects(objects) {
  ...
  objects.forEach((item) => {
    const arrKeys = Object.entries(item);
    arrKeys.forEach((i) => {
      if (newObj[i[0]] === undefined) {
        newObj[i[0]] = item[i[0]];
      } else {
        newObj[i[0]] += item[i[0]];
      }
...
function makeWord(lettersObject) {
  let resString = '';
  const arrObjValues = Object.values(lettersObject);
  const arrObjKeys = Object.keys(lettersObject);
  let flatArrObjValues = [];
  arrObjValues.forEach((item) => {
    flatArrObjValues = flatArrObjValues.concat(item);
  });
  flatArrObjValues.forEach((item, index) => {
    arrObjKeys.forEach((i) => {
      if (lettersObject[i].includes(index)) {
        resString += i;
      }
    });
  });
...
function sellTickets(queue) {
  const register = [];
  const arrMoneyRender = (nominal) => {
    const resArr = [];
    for (let i = 0; i < register.length; i += 1) {
      if (register[i] === nominal) {
        resArr.push(nominal);
      }
    }
    return resArr;
  ...
  queue.forEach((item) => {
    if (item === 25) {
      ...
      if (register.includes(25) && register.includes(50)) {
        ...
        register.push(100);
      } else if (arrMoneyRender(25).length >= 3) {
        register.splice(register.indexOf(25), 1);
        register.splice(register.indexOf(25), 1);
        register.splice(register.indexOf(25), 1);
        register.push(100);
...
function fromJSON(proto, json) {
  const arg = Object.keys(JSON.parse(json));
  const value = Object.values(JSON.parse(json));
  class NewClass {
    constructor(arrValue) {
      const arr = [...arg];
      for (let i = 0; i < arr.length; i += 1) {
        this[arr[i]] = arrValue[i];
      }
    }
  }
  return Object.setPrototypeOf(new NewClass([...value]), proto);
}
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((item) => {
    if (map.has(keySelector(item))) {
      const curCityArr = map.get(keySelector(item));
      curCityArr.push(valueSelector(item));
      map.set(keySelector(item), curCityArr);
    } else {
      const curCityArr = [valueSelector(item)];
      map.set(keySelector(item), curCityArr);
    }
  });
